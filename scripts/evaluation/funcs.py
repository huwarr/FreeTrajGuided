...
def batch_ddim_inversion(model, cond, latents, ddim_steps=50, ddim_eta=1.0, cfg_scale=1.0, temporal_cfg_scale=None, **kwargs):
    ...
    samples, intermediates = ddim_sampler.inverse(S=ddim_steps,
                                    conditioning=cond,
                                    batch_size=shape[0],
                                    shape=shape[1:],
                                    x0=latents,
...
def batch_ddim_sampling_freetraj_with_path(model, cond, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1.0,\
                        cfg_scale=1.0, temporal_cfg_scale=None, idx_list=[], paths=[], x_T_total=None, args=None, **kwargs):
    ddim_sampler = DDIMFreeTrajSampler(model)
    uncond_type = model.uncond_type
    batch_size, channels, frames, h, w = noise_shape
...
def batch_ddim_sampling_freetraj_with_path_cmaps_selfattn(model, cond, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1.0,\
                        cfg_scale=1.0, temporal_cfg_scale=None, idx_list=[], paths=[], x_T_total=None, args=None, cmaps=[], **kwargs):
    ddim_sampler = DDIMFreeTrajSampler(model)
    uncond_type = model.uncond_type
    batch_size, channels, frames, h, w = noise_shape
...
def batch_ddim_sampling_freetraj_with_cmaps_selfattn(model, cond, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1.0,\
                        cfg_scale=1.0, temporal_cfg_scale=None, idx_list=[], input_traj=[], x_T_total=None, args=None, cmaps=[], **kwargs):
    ddim_sampler = DDIMFreeTrajSampler(model)
    uncond_type = model.uncond_type
    batch_size, channels, frames, h, w = noise_shape
    ...
    for idx, vid_tensor in enumerate(batch_tensors):
        ...
        for i in range(video.shape[1]):
            ...
            for j in range(video.shape[0]): 
                ...
                grid[j, h_start-1:h_start+2, w_start-1:w_end+1, :] = torch.ones_like(grid[j, h_start-1:h_start+2, w_start-1:w_end+1, :]) * torch.Tensor([127, 255, 127]).view(1, 1, 3)
                grid[j, h_end-2:h_end+1, w_start-1:w_end+1, :] = torch.ones_like(grid[j, h_end-2:h_end+1, w_start-1:w_end+1, :]) * torch.Tensor([127, 255, 127]).view(1, 1, 3)
            bbox_savepath = os.path.join(bboxdir, f"{filenames[idx]}_{str(i)}.mp4")
            torchvision.io.write_video(bbox_savepath, grid, fps=fps, video_codec='h264', options={'crf': '10'})

    # reference
    # B x C x F x H x W
    ref_video = torch.clamp(ref_video.cpu().float(), -1., 1.)
    ref_video = ref_video[0].permute(1, 0, 2, 3) # t,c,h,w
    h_len = ref_video.shape[2]
    w_len = ref_video.shape[3]
    sub_h = int(BOX_SIZE_H * h_len) 
    sub_w = int(BOX_SIZE_W * w_len)

    frame_grids = [torchvision.utils.make_grid(framesheet, nrow=1) for framesheet in ref_video] #[3, 1*h, n*w]
    grid = torch.stack(frame_grids, dim=0) # stack in temporal dim [t, 3, n*h, w]
    grid = (grid + 1.0) / 2.0
    grid = (grid * 255).to(torch.uint8).permute(0, 2, 3, 1)
    savepath = os.path.join(refdir, "reference_0.mp4")
    torchvision.io.write_video(savepath, grid, fps=fps, video_codec='h264', options={'crf': '10'})

    for j in range(ref_video.shape[0]): 
        h_start = int(paths[j][0] * h_len)
        h_end = h_start + sub_h
        w_start = int(paths[j][2] * w_len)
        w_end = w_start + sub_w
        ...
        grid[j, h_start-1:h_start+2, w_start-1:w_end+1, :] = torch.ones_like(grid[j, h_start-1:h_start+2, w_start-1:w_end+1, :]) * torch.Tensor([127, 255, 127]).view(1, 1, 3)
        grid[j, h_end-2:h_end+1, w_start-1:w_end+1, :] = torch.ones_like(grid[j, h_end-2:h_end+1, w_start-1:w_end+1, :]) * torch.Tensor([127, 255, 127]).view(1, 1, 3)
    bbox_savepath = os.path.join(refdir, "reference_0_bbox.mp4")
    torchvision.io.write_video(bbox_savepath, grid, fps=fps, video_codec='h264', options={'crf': '10'})